...
const ProductSlider = () => {
  ...
  const properties = {
    prevArrow: <button className="hidden bg-gray-300 rounded-[50%] p-2 text-xl lg:block"> <GrPrevious></GrPrevious> </button>,
    nextArrow: <button className="hidden bg-gray-300 rounded-[50%] p-2 text-xl lg:block"> <GrNext/> </button>,
  };
  ...
  return (
   <> 
      ...
      {products.map((product) => (
        <div key={product._id} className="overflow-hidden h-[350px] m-2 ">
          <div className="bg-gray-100 border border-gray-400 rounded-md w-full h-full ">
             ...
             {/* card images */}
            <div style={{backgroundImage : `url(${product.images[1].url})`}} className="w-full h-[70%] bg-no-repeat overflow-hidden  bg-cover bg-center   "></div>
          ...
          {/* card details */}
            <div className="w-full h-[30%] border-t border-gray-400  p-1 px-2 flex flex-col justify-evenly ">
              ...
              <div className="flex gap-2 items-center">
                <p className=" font-sans text-[18px]  ">{"$"+ product.price}</p>
                <p className=" font-sans text-[15px] line-through text-gray-600 ">
                  {"$"+ Math.ceil(product.price*130/100)}
                </p>
                <p className=" font-sans text-[17px] text-green-500   ">30%</p>
              </div>
     ...
     ( 
      <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" className="w-12 h-12 mt-8 animate-spin"
    viewBox="0 0 16 16">
    <path
